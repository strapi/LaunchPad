...
/**
 * `react-fast-marquee` is a CJS module that uses `exports.default = Marquee`.
 * When Node's ESM → CJS interop loads it, `import X from 'react-fast-marquee'`
 * can return either the component function directly OR `{ default: Fn }`
 * depending on the loader path. Under TanStack Start's route-split SSR it
 * comes back as the latter, so we unwrap defensively at module scope.
 */
const Marquee = ((MarqueeModule as any).default ??
  MarqueeModule) as React.ComponentType<{
            ...
            >
              ...
              <div className="flex gap-2 items-center mt-8">
                <StrapiMedia
                  ...
                  mime={testimonial?.user?.image?.mime}
