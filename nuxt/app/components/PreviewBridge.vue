...
const handleMessage = (message: MessageEvent) => {
  ...
  if (data.type === 'strapiUpdate') {
    ...
    // The admin answers every previewReady and never retries. A second inject
    // is fatal: the script is not idempotent. Run 2 appends a new highlight
    // stylesheet, then calls run 1's stale cleanup, which deletes it by id.
    // Zero stylesheets remain, the highlights inherit pointer-events:none, and
    // every click passes straight through them.
